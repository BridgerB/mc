...
nix build .#nixosConfigurations.velocity.config.system.build.toplevel
```
...
./deploy.ts 144.24.32.76 velocity
```
...
3. Copy the shared files (flake.nix, common.nix, config.nix) and the
   velocity/ files to `/etc/nixos/`
4. Run `nixos-rebuild switch --flake .#velocity`
...
# On the OCI instance
cd /etc/nixos
# Copy your configuration files here (preserving the velocity/ subdir layout)

nixos-rebuild switch --flake .#velocity
```
...
Shared files live one level up in `server/oci/` (imported by both this config
and the steve config); velocity-specific files live in this directory.

In `server/oci/` (shared):

- **flake.nix** - Defines `nixosConfigurations.velocity` and `.steve`, plus the
  inline Advanced Portals build and Paper/Velocity from nixpkgs
- **flake.lock** - Pinned nixpkgs and Advanced Portals source
- **common.nix** - Shared base config (SSH, fail2ban, root keys, dev tooling)
- **hardware-configuration.nix** - Placeholder; real one generated on server
- **config.nix** - SSH keys (user-specific)
- **config.ts** - OCI identifiers for the TypeScript scripts (user-specific)
- **launch.ts** - Deno script to launch an OCI instance (requires OCI CLI)
- **deploy.ts** - Deno script: `./deploy.ts <ip> velocity|steve`

In `server/oci/velocity/` (this directory):

- **configuration.nix** - Velocity proxy + Paper sub-worlds (imports ../common.nix)
- **velocity.toml** - Velocity proxy configuration
- **portals/** - Advanced Portals lobby portal definitions
- **backup.sh** - Download all world saves from the server
- **WORLDS.md** - Per-world notes
